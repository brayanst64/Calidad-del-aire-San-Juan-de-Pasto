...
char *root_topic_subscribe = "XzblrWRu1CApIWw/Dispositivo3";
char *root_topic_publish = "XzblrWRu1CApIWw/Dispositivo3";
...
float RL_4 = 12840;     // Resistencia de carga en Khom, recomendada 20k (10k a 47k)
float RO_4 = -19442.93;   // Resistecia de calibracion hom


// Variables variables sensor MQ-7 CO Monoxido de Carbono
float MQ7;
int gas_sensor_7 = 0;  // pin ADC
float RL_7 = 9650;      // Resistencia de carga en Khom, recomendada 10k (5k a 47k)
float RO_7 = 34080.96;    // Resistecia de calibracion hom
...
float RO_131 = 92.86;    // Resistencia de calibracion en hom
...
void setup_wifi() {
  ...
  WiFi.begin("RODRIGUEZ B", "140587503332");
...
void reconnect() {
  ...
  while (!client.connected() && veces <= intento) {
    ...
    String clientId = "MEDIDOR_GASES3";
...
void loop() {
  ...
  if (tiempo2 >= 60000 && s2 == false) {
    ...
    if (medicion >= 1) {
      ...
      File archivo = SD.open("/Estacion3.txt", FILE_APPEND);
      if (archivo) {
  ...
  if (s3 == true)
  {
    ...
    // Sensor MG811
    Prom_mg811 = (acum4 / muestras) * 1000 / 5.7 ;
    if (Prom_mg811 <= 265)
    {
      MG811 = 10000;
    }
