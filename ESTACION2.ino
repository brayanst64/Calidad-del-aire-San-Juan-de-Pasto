...
char *root_topic_subscribe = "XzblrWRu1CApIWw/Dispositivo2";
char *root_topic_publish = "XzblrWRu1CApIWw/Dispositivo2";
...
float RL_4 = 18000;     // Resistencia de carga en Khom, recomendada 20k (10k a 47k)
float RO_4 = -38268.62;   // Resistecia de calibracion hom
...
float RO_7 = 110107.73;    // Resistecia de calibracion hom
...
float RO_131 = 454.34;   // Resistencia de calibracion en hom
...
void reconnect() {
  ...
  while (!client.connected() && veces <= intento) {
    ...
    String clientId = "MEDIDOR_GASES2";
...
void setup() {
  ...
  Serial.println("inicializacion SD correcta");  // texto de inicializacion correcta
...
void loop() {
  ...
  if (tiempo2 >= 60000 && s2 == false) {
    s2 = true;
    s4 = false;
    ledcWrite(LEDC_CHANNEL_0, 2293);
    if (medicion >= 6) {
      medicion = 0;
      DateTime now = rtc.now();
      File archivo = SD.open("/Estacion2.txt", FILE_APPEND);
      if (archivo) {
  ...
  while (tiempo2 >= 140000 && s4 == false ) {
    ...
    acum4 = acum4 + V3;
    V5 = float(analogRead(gas_sensor_8511)) * 3.3 / 4095;
    acum5 = acum5 + V5;
    muestras++;
    if (muestras == 100) {
      s3 = true;
  ...
  if (s3 == true)
  {
    ...
    // Sensor MG811
    Prom_mg811 = (acum4 / muestras) * 1000 / 9.84 ;
    Prom_mg811 = Prom_mg811 + 158.41;
    if (Prom_mg811 <= 265)
    {
      MG811 = 10000;
    }
